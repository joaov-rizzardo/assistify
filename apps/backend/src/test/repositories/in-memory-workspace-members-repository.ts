...
export class InMemoryWorkspaceMembersRepository implements WorkspaceMembersRepository {
  ...
  async findWorkspaceMember(workspaceId: string, userId: string): Promise<WorkspaceMember> {
    const workspaceMember = this.workspaceMembers.find((workspace) => workspace.getWorkspaceId() === workspaceId && workspace.getUserId() === userId);
    return workspaceMember || null;
  }

  async findUserWorkspaces(userId: string): Promise<WorkspaceMember[]> {
    const workspaces = await this.workspaceMembers.filter((workspace) => workspace.getUserId() === userId);
    return workspaces;
  }

  async remove(userId: string, workspaceId: string): Promise<void> {
    this.workspaceMembers = this.workspaceMembers.filter((member) => member.getUserId() !== userId && member.getWorkspaceId() !== workspaceId);
  }
  ...
  async changeMemberRole(userId: string, workspaceId: string, role: WorkspaceMemberRoles): Promise<WorkspaceMember> {
    this.workspaceMembers = this.workspaceMembers.map((member) => {
      if (member.getUserId() === userId && member.getWorkspaceId() === workspaceId) {
    ...
    return this.workspaceMembers.find((member) => member.getUserId() === userId && member.getWorkspaceId() === workspaceId) || null;
  }

  changeMemberStatus(userId: string, workspaceId: string, status: WorkspaceMemberStatus): WorkspaceMember | null {
    const workspaceMember = this.workspaceMembers.find((member) => member.getUserId() === userId && member.getWorkspaceId() === workspaceId);
  ...
  checkIfMemberExists(workspaceId: string, userId: string): boolean {
    return this.workspaceMembers.some((member) => member.getUserId() === userId && member.getWorkspaceId() === workspaceId);
  }
}
