...
describe('Get user basic info (E2E)', () => {
  ...
  test('[GET] /users/basic/:id it should return user basic info', async () => {
    const response = await request(app.getHttpServer()).get(`/users/basic/${user.getId()}`).set('Authorization', `Bearer ${accessToken}`);
  ...
  test('[GET] /users/basic/:id it should check if user exists', async () => {
    const response = await request(app.getHttpServer()).get(`/users/basic/${uuid()}`).set('Authorization', `Bearer ${accessToken}`);
    expect(response.statusCode).toBe(404);
    expect(response.body.code).toBe('USER_NOT_EXISTS');
  });

  test('[GET] /users/basic/:id it should return unknown user basic info', async () => {
    const anotherUser = await userFactory.makePrismaUser();
    const response = await request(app.getHttpServer()).get(`/users/basic/${anotherUser.getId()}`).set('Authorization', `Bearer ${accessToken}`);
    expect(response.statusCode).toBe(200);
