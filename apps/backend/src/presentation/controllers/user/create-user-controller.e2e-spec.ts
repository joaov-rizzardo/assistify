...
describe('Create user (E2E)', () => {
  ...
  test('[POST] /users it should create new user', async () => {
    ...
    const response = await request(app.getHttpServer()).post('/users').send({ name, email, lastName, password });
    expect(response.body).toHaveProperty('id');
    const userId = response.body.id;
    const userHaveBeenCreated = await userRepository.checkIfUserExistsById(userId);
  ...
  test('[POST] /users it should check if another user with same email already exists', async () => {
    ...
    const response = await request(app.getHttpServer()).post('/users').send({ name, email: user.getEmail(), lastName, password });
