...
export class GetUserUnreadNotificationsController {
  constructor(private readonly getUnreadUserNotificationsUseCase: GetUnreadUserNotificationsUseCase) {}
  ...
  async handle(@Req() req: UserRequest) {
    ...
    const notifications = await this.getUnreadUserNotificationsUseCase.execute(userId);
    return notifications.map((notification) => UserNotificationPresenter.toHTTP(notification));
  }
}
