...
describe('Get user workspaces (E2E)', () => {
  ...
  test('[GET] /workspaces/basic/:id it should return workspace basic info', async () => {
    const response = await request(app.getHttpServer()).get(`/workspaces/basic/${workspace.getId()}`).set('Authorization', `Bearer ${accessToken}`);
    ...
    expect(returnedWorkspace.createdAt).toBe(workspace.getCreatedAt().toISOString());
  });

  test('[GET] /workspaces/basic/:id it should check if workspace exists', async () => {
    const response = await request(app.getHttpServer()).get(`/workspaces/basic/${uuid()}`).set('Authorization', `Bearer ${accessToken}`);
