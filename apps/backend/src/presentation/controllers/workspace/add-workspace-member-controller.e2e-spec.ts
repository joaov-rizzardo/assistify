...
describe('Add workspace member (E2E)', () => {
  ...
  test('[POST] /workspaces/member/add it should add workspace member', async () => {
    ...
    const addedMember = await workspaceMembersRepository.findWorkspaceMember(workspace.getId(), userMember.getId());
  ...
  test('[POST] /workspaces/member/add it should invite user to workspace', async () => {
    ...
    const inviteNotification = notifications.find((notification) => {
      ...
      if (content.type === 'workspace_invite' && content.workspaceId === workspace.getId() && content.invitingUserId === user.getId()) {
        return true;
      }
      return false;
    });
    expect(inviteNotification).toBeTruthy();
    expect(userSocketEmitter.sendToUser).toHaveBeenCalledWith(userMember.getId(), 'USER_NOTIFICATION', inviteNotification.toObject());
  });
