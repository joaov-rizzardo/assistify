...
export class UpdateWorkspaceController {
  constructor(private readonly updateWorkspaceUseCase: UpdateWorkspaceUseCase) {}
  ...
  async handle(@Body() args: UpdateWorkspaceDTO, @Req() req: WorkspaceRequest) {
    const result = await this.updateWorkspaceUseCase.execute(req.workspace.id, args);
    if (result.isLeft()) {
