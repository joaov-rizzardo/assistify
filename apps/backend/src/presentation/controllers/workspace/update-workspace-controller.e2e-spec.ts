...
import { WorkspaceFactory, makeWorkspace } from 'src/test/factories/make-workspace';
...
describe('Update workspace (E2E)', () => {
  ...
  test('[PATCH] /workspaces it should update workspace', async () => {
    ...
    const updatedWorkspace = await workspaceRepository.findById(createdWorkspace.getId());
    expect(updatedWorkspace.getName()).toBe(newName);
    expect(new Date(updatedWorkspace.getUpdatedAt())).not.toEqual(new Date(createdWorkspace.getUpdatedAt()));
  });
  ...
  test('[PATCH] /workspaces it should return updated workspace', async () => {
    ...
    expect(new Date(updatedWorkspace.createdAt)).toEqual(new Date(createdWorkspace.getCreatedAt()));
  });
});
