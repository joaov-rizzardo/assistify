import { BadRequestException, Controller, NotFoundException, Param, Patch, Req, UnauthorizedException, UseGuards } from '@nestjs/common';
...
export class RejectWorkspaceInviteController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Response when workspace invite was rejected successfully'
  })
  @ApiResponse({
    status: 404,
  ...
  async handle(@Param('notificationId') notificationId: string, @Req() req: UserRequest) {
    const notification = await this.userNotificationRepository.findById(notificationId);
    if (!notification) {
    ...
    const result = await this.responseWorkspaceInviteUseCase.execute(notification, 'reject');
    if (result.isLeft()) {
