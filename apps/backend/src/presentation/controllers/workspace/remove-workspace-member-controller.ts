import { BadRequestException, Controller, Delete, Param, Req, UseGuards } from '@nestjs/common';
...
export class RemoveWorkspaceMemberController {
  constructor(private readonly removeWorkspaceMemberUseCase: RemoveWorkspaceMemberUseCase) {}
  ...
  async remove(@Param('userId') userId: string, @Req() request: WorkspaceRequest) {
    const workspaceId = request.workspace.id;
    const result = await this.removeWorkspaceMemberUseCase.execute(workspaceId, userId);
    if (result.isLeft()) {
