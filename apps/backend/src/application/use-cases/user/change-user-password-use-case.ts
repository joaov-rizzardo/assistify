...
type CreateUserUseCaseResponse = Either<UserAlreadyExistsError | IncorrectUserPasswordError, User>;
...
export class ChangeUserPasswordUseCase {
  ...
  async execute({ currentPassword, newPassword, userId }: UpdatePasswordArgsType): Promise<CreateUserUseCaseResponse> {
    const user = await this.userRepository.findById(userId);
    if (!user) {
      return left(new UserNotExistsError(userId));
    }
    if (!(await this.passwordEncrypter.checkPassword(currentPassword, user.getPassword()))) {
      return left(new IncorrectUserPasswordError(userId));
    }
    const updatedUser = await this.userRepository.changePassword(userId, newPassword);
    if (!updatedUser) {
      return left(new UserNotExistsError(userId));
    }
