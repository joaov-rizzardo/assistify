...
export type AddWorkspaceMemberUseCaseResponse = Either<
  UserNotExistsError | CannotAddMemberAsOwnerError | UserIsAlreadyWorkspaceMemberError,
  WorkspaceMember
>;
...
export class AddWorkspaceMemberUseCase {
  ...
  public async execute(workspaceId: string, { userId, role, invitingUserId }: AddMemberArgs): Promise<AddWorkspaceMemberUseCaseResponse> {
    if (role === 'owner') {
    ...
    let member: WorkspaceMember = await this.workspaceMembersRepository.findWorkspaceMember(workspaceId, userId);
