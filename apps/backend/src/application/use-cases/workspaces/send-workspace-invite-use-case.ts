...
export class SendWorkspaceInviteUseCase {
  ...
  async execute({ invitingUserId, userId, workspaceId }: CreateArgsType) {
    let notification: UserNotification | null = await this.userNotificationRepository.findWorkspaceInvite(userId, workspaceId);
    if (notification) {
      notification = await this.userNotificationRepository.update(notification.getId(), {
        date: new Date()
      });
    }
    if (!notification) {
    ...
    this.userSocket.sendToUser(userId, UserSocketEventsInput.Notification, notification.toObject());
    return notification;
  }
}
