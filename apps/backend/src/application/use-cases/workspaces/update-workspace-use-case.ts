...
export type UpdateWorkspaceUseCaseResponse = Either<WorkspaceNotExistsError, Workspace>;

@Injectable()
export class UpdateWorkspaceUseCase {
  constructor(private readonly workspaceRepository: WorkspaceRepository) {}
  async execute(workspaceId: string, args: Partial<UpdateWorkspaceDTO>): Promise<UpdateWorkspaceUseCaseResponse> {
    const updatedWorkspace = await this.workspaceRepository.update(workspaceId, args);
    if (!updatedWorkspace) {
