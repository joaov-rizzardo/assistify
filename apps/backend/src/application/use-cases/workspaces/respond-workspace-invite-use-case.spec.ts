...
describe('Respond workspace invite use case', () => {
  ...
  beforeEach(async () => {
    ...
    sut = new RespondWorkspaceInviteUseCase(workspaceMembersRepository, userNotificationRepository);
  });
  ...
  it('should accept workspace invite', async () => {
    ...
    const updatedMember = await workspaceMembersRepository.findWorkspaceMember(workspaceMember.getWorkspaceId(), workspaceMember.getUserId());
  ...
  it('should check if notification is a workspace invite', async () => {
    ...
    if (result.isLeft()) {
      expect(result.value).toBeInstanceOf(NotificationIsNotWorkspaceInviteError);
    }
  });
