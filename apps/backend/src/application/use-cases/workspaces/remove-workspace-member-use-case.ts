...
export type RemoveWorkspaceMemberUseCaseResponse = Either<UserIsNotMemberFromWorkspaceError | CannotRemoveOwnerFromWorkspaceError, boolean>;
...
export class RemoveWorkspaceMemberUseCase {
  ...
  async execute(workspaceId: string, userId: string): Promise<RemoveWorkspaceMemberUseCaseResponse> {
    const member = await this.workspaceMemberRepository.findWorkspaceMember(workspaceId, userId);
    if (!member) {
