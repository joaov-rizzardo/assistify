...
export class AuthenticateUserUseCase {
  ...
  async execute({ email, password }: AuthenticateUserDTO): Promise<AuthenticateUserUseCaseResponse> {
    const user = await this.userRepository.findByEmail(email);
    if (user === null) {
      return left(new AuthenticationBadCredentialsError());
    }
    if ((await this.passwordEncryter.checkPassword(password, user.getPassword())) === false) {
      return left(new AuthenticationBadCredentialsError());
    }
