...
export class PrismaUserNotificationRepository implements UserNotificationRepository {
  ...
  async create({ type, userId, content }: CreateUserNotificationArgs): Promise<UserNotification> {
  ...
  async find(args: FindNotificationArgs): Promise<UserNotification[]> {
    ...
    return notifications.map((notification) => this.instanceUserNotificationByReturnQuery(notification));
  }
  ...
  async findWorkspaceInvite(userId: string, workspaceId: string): Promise<UserNotification | null> {
  ...
  async update(notificationId: string, args: UpdateUserNotificationArgs): Promise<UserNotification | null> {
    const notification = await this.prisma.client.userNotifications.update({
      data: {
        ...args
      },
  ...
  async findUserUnreadNotifications(userId: string): Promise<UserNotification[]> {
    const notifications = await this.prisma.client.userNotifications.findMany({
      where: {
        user_id: userId,
        read: false
  ...
  private instanceUserNotificationByReturnQuery(notification: PrismaNotification) {
