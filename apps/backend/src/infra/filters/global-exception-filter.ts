...
export class GlobalExceptionFilter implements ExceptionFilter {
  ...
  async catch(exception: unknown, host: ArgumentsHost) {
    ...
    const httpResponse = exception instanceof HttpException ? exception.getResponse() : { message: 'Internal server error' };

    const errorMessage = exception instanceof HttpException || exception instanceof Error ? exception.message : 'Unknown error';

    const errorStack = exception instanceof HttpException || exception instanceof Error ? exception.stack : undefined;

    await this.httpLogger.log(this.getErrorLevelByHttpCode(httpStatus), errorMessage, {
      httpCode: httpStatus,
