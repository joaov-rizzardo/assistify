...
import {
  ...
  BaileysSessionUpsertType,
} from 'src/infra/libs/baileys/repositories/baileys-session-repository';
import { PrismaProvider } from '../prisma-provider';
import { BaileysSession } from 'src/infra/libs/baileys/entities/baileys-session';

@Injectable()
export class PrismaBaileysSessionRepository
  implements BaileysSessionRepository
{
  constructor(private readonly prisma: PrismaProvider) {}

  public async deleteByIdAndSessionId(
    id: string,
    sessionId: string,
  ): Promise<void> {
    await this.prisma.baileysSession.delete({
      select: { pkId: true },
      where: {
  ...
  public async upsert({
    ...
    const session = await this.prisma.baileysSession.upsert({
      create: { data, id, sessionId },
      update: { data },
      where: { sessionId_id: { id, sessionId } },
    });
    return new BaileysSession(
      session.pkId,
      session.id,
      session.sessionId,
      session.data,
  ...
  public async findActiveSessions(): Promise<BaileysSession[]> {
    const sessions = await this.prisma.baileysSession.findMany({
      where: { id: 'creds' },
    });
    return sessions.map(
      (session) =>
  ...
  public async deleteSessionById(sessionId: string): Promise<void> {
    await this.prisma.baileysSession.deleteMany({
      where: { sessionId },
    });
  }
}
