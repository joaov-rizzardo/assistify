...
export class BaileysStore {
  ...
  public async getState() {
    this.creds =
      ((await this.read('creds')) as AuthenticationCreds) || initAuthCreds();
    const state = {
      creds: this.creds,
      keys: {
  ...
  private async write(data: any, id: string) {
    try {
  ...
  private async read(id: string) {
    try {
      id = this.fixId(id);
      const session = await this.sessionRepository.findByIdAndSessionId(
        id,
        this.sessionId,
      );
      if (!session) {
