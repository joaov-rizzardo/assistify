...
describe("Get user's unread notifications (E2E)", () => {
  ...
  beforeAll(async () => {
    ...
    unreadNotification = await userNotificationFactory.makeUserNotification(
      user.getId(),
      {
        read: false,
  ...
  test('[GET] /notifications/user/unread it should return unread user notifications', async () => {
    ...
    expect(new Date(firstNotification.date)).toEqual(
      unreadNotification.getDate(),
    );
    expect(new Date(firstNotification.createdAt)).toEqual(
      unreadNotification.getCreatedAt(),
    );
    expect(new Date(firstNotification.updatedAt)).toEqual(
      unreadNotification.getUpdatedAt(),
    );
