...
describe('Accept workspace invite (E2E)', () => {
  ...
  beforeEach(async () => {
    ...
    await workspaceMemberFactory.makePrismaWorkspaceMember(
      workspace.getId(),
      memberUser.getId(),
      {
        role: 'editor',
  ...
  test('[PATCH] /workspaces/:notificationId/accept it should accept workspace invite', async () => {
    const notification = await userNotificationFactory.makeUserNotification(
      memberUser.getId(),
      {
        read: false,
    ...
    const response = await request(app.getHttpServer())
      .patch(`/workspaces/${notification.getId()}/accept/`)
  ...
  test('[PATCH] /workspaces/:notificationId/accept it should check if notification exists', async () => {
    const response = await request(app.getHttpServer())
      .patch(`/workspaces/${uuid()}/accept/`)
      .set('Authorization', `Bearer ${accessToken}`);
    expect(response.statusCode).toBe(404);
    expect(response.body.code).toBe('NOTIFICATION_NOT_EXISTS');
  });

  test('[PATCH] /workspaces/:notificationId/accept it should check if user is authorized to access this resource', async () => {
    const notification = await userNotificationFactory.makeUserNotification(
      memberUser.getId(),
      {
        read: false,
  ...
  test('[PATCH] /workspaces/:notificationId/accept it should check if invite is valid', async () => {
    ...
    const updatedNotification = await userNotificationRepository.findById(
      notification.getId(),
    );
