...
type RespondWorkspaceInviteUseCaseResponse = Either<
  ...
  | WorkspaceInviteIsNotValidError
  | WorkspaceInviteIsAlreadyAcceptedError,
  UserNotification
>;
...
export class RespondWorkspaceInviteUseCase {
  ...
  async execute(
  ...
  ): Promise<RespondWorkspaceInviteUseCaseResponse> {
    if (
      notification.getType() !== 'workspace_invite' ||
      notification.getContent()?.type !== 'workspace_invite'
    ) {
      return left(
        new NotificationIsNotWorkspaceInviteError(notification.getId()),
      );
    }
    const workspaceId = notification.getContent().workspaceId;
    const updatedNotification = await this.userNotificationRepository.read(
      notification.getId(),
    );
    ...
    if (!workspaceMember) {
      return left(
        new WorkspaceInviteIsNotValidError(
          notification.getId(),
          notification.getUserId(),
          workspaceId,
        ),
    ...
    if (workspaceMember.getStatus() === 'accepted') {
      return left(
        new WorkspaceInviteIsAlreadyAcceptedError(
          notification.getUserId(),
          workspaceId,
        ),
      );
    ...
    if (operation === 'accept') {
      await this.workspaceMembersRepository.changeMemberStatus(
        notification.getUserId(),
        workspaceId,
        'accepted',
      );
    ...
    if (operation === 'reject') {
      await this.workspaceMembersRepository.remove(
        notification.getUserId(),
        workspaceId,
      );
    }
